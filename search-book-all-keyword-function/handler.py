...
def lambda_handler(event, context):
    ...
    # compile query condition string
    ...
    # execute query, fetch result from MySQL database
    cursor = connection.cursor()
    
    if initialIndex == "0":
        sql = "SELECT i.item_id, b.book_title, b.book_author, c.collection_name, b.edition, b.publishing_year, a.availability_status_name, l.library_name, i.call_number, b.image_url FROM book_info AS b " + "JOIN item_info AS i ON b.book_id=i.book_id " + "JOIN collection_info AS c on i.collection_id=c.collection_id JOIN availability_status AS a ON a.availability_status_id=i.availability_status_id JOIN library_info AS l ON l.library_id=i.library_id" + " WHERE (b.book_title LIKE '%"+ keyword + "%' OR b.book_author LIKE '%" + keyword + "%' OR b.isbn LIKE '%" + keyword + "%' OR b.publication_info LIKE '%" + keyword + "%' OR b.content_summary LIKE '%" + keyword + "%' OR b.book_content LIKE '%" + keyword + "%' OR i.barcode LIKE '%" + keyword + "%')" + availabilityQueryPart + collectionQueryPart + minYearQueryPart + maxYearQueryPart + subjectQueryPart   
        cursor.execute(sql)
